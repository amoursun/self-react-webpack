        ...
        return (
            <div className="cell">
                <div className="cell__child-container">
                    <Link to={`newShopTable/update/${name}`}><span>修改</span></Link>&nbsp;| &nbsp;
                    <Link to={`newShopTable/copy/${name}`}><span>复制</span></Link>&nbsp;| &nbsp;
...
export default class Tr extends PureComponent {
    ...
    render() {
        ...
        return (<div className="tr">
            ...
            {withRouter(<Option onDelete={onDelete} name={data.get('name')}/>)}
        </div>);
    }
}